  ...
  return (
    <>
      ...
      {activeProject && (
        ...
        >
          {/* Main Card Wrapper imitating the image style */}
          <div 
            className="relative w-full max-w-[340px] h-[580px] rounded-[44px] shadow-2xl overflow-hidden shadow-black/30 border-[6px] border-white flex flex-col justify-end animate-in fade-in zoom-in-95 duration-300"
          >
            {/* Background Image */}
            ...
            <button 
              ...
              className="absolute top-4 right-4 w-12 h-12 rounded-full bg-white/20 backdrop-blur-md border border-white/30 flex items-center justify-center text-white hover:bg-white/40 transition-colors z-20"
            >
              <X size={20} className="drop-shadow-sm" />
            </button>
            <button 
              className="absolute top-4 left-4 w-12 h-12 rounded-full bg-white/20 backdrop-blur-md border border-white/30 flex items-center justify-center text-white hover:bg-white/40 transition-colors z-20"
            >
              <Heart size={20} className="drop-shadow-sm" />
            </button>

            {/* Bottom Info Gradient Overlay & Content */}
            <div className="w-full relative z-10 pt-20 pb-4 px-5">
              {/* Blur/Gradient Background Layer */}
              <div className="absolute top-0 left-0 right-0 bottom-0 bg-linear-to-t from-black/90 via-black/50 to-transparent pointer-events-none rounded-b-[36px]" />
              <div className="absolute top-0 left-0 right-0 bottom-0 backdrop-blur-[6px] mask-image-linear-to-b from-transparent to-black pointer-events-none rounded-b-[36px]" style={{ WebkitMaskImage: 'linear-gradient(to bottom, transparent, black 100%)' }} />

              <div className="relative z-20 flex flex-col gap-4">
                {/* Titles */}
                <div>
                  <h2 className="text-white text-[32px] font-bold leading-tight mb-1 drop-shadow-md tracking-tight">
                ...
                <div className="flex items-center gap-5 text-white/90 font-medium">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-[15px]">from <strong className="text-white ml-0.5">{activeProject.budget.split('-')[0]}</strong></span>
                  </div>
                  <div className="flex items-center gap-2">
                    <LinkIcon size={16} className="opacity-80" />
                    <span className="text-[15px] underline underline-offset-2 decoration-white/40 hover:decoration-white transition-colors cursor-pointer text-white">
                      View details
                    </span>
                  </div>
                </div>
                ...
                <a 
                  ...
                  className="w-full bg-white text-black text-[15px] font-bold py-4 rounded-full flex items-center justify-center hover:bg-gray-100 transition-colors shadow-lg mt-1"
                >
                  Demo Links
                </a>
              </div>
